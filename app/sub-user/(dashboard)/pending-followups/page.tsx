...
export default function Page() {
    ...
    const { data, error, isLoading } = useSWR(`/api/sub-user/dashboard/pending-followups`, fetcher);
    ...
    const handleDelete = async (inquiryIds: number[]) => {
        try {
            ...
            mutate("/api/sub-user/dashboard/pending-followups");
            setSelectedRows({});
    ...
    const columns: Column<DataItem>[] = [
        {
            ...
            accessor: (item) => (
                <div className="flex items-center gap-2 group">
                    <Link href={`/sub-user/pending-followups/${item.inquiry.id}`}><span className="truncate">{item.inquiry?.name}</span></Link>
        ...
        },
        {
            ...
            accessor: (item) => (
                item.inquiry?.phone ? (<div className="flex items-center gap-2 group">
                    <Link href={`/sub-user/pending-followups/${item.inquiry.id}`}>
        ...
        },
        {
            ...
            accessor: (item) => (
                <div className="flex items-center gap-2 group relative ">
                    <Link href={`/sub-user/pending-followups/${item.inquiry.id}`}>
                        <span className="truncate text-red-500">{item.followUpStatus === "Assign To" ? `Assign To ${item.assignToName}` : item.followUpStatus}</span>
        ...
        <section>
            ...
            <div >
                <DataTableComponent
                    ...
                    detail={(item) => `/sub-user/pending-followups/${item.inquiry.id}`}
