...
export default function Page() {
    ...
    const params = useParams<{ user_id: string; inquiry_id: string; domain_id: string }>();
    const [followup, setFollowup] = useState(false);
    const { user_id, inquiry_id , domain_id } = params;
            ...
            <div className="flex justify-end py-2 px-3">
                <CustomBreadcrumb
                    paths={[
                        ...
                        { label: "Records", href: `/user/inquiry/view/${domain_id}` },
                        { label: "Detail", isPage: true },
                    ]}
                />
            </div>

            {followup ? <OverView back={`/user/inquiry/view/${domain_id}`} id={inquiry_id} /> : <TimeLine inquiryId={inquiry_id}  />}
        </>
    )
}