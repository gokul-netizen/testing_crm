...
export default function Page() {
    ...
    const { data, error, isLoading } = useSWR(`/api/user/dashboard/closed-followups`, fetcher);
    ...
    const columns: Column<DataItem>[] = [
        {
            ...
            accessor: (item) => (
                <div className="flex items-center gap-2 group">
                    <Link href={`/user/closed-followups/${item.id}`}>
        ...
        },
        {
            ...
            accessor: (item) => (
                <div className="flex items-center gap-2 group relative ">
                    <Link href={`/user/closed-followups/${item.id}`} className="flex flex-col">
                        <span className="truncate text-red-500">{item.followUpStatus}</span>
        ...
        <section>
            ...
            <div  >
                <DataTableComponent
                    ...
                    detail={(item) =>  `/user/closed-followups/${item.id}`}
