...
export default function Page() {
    ...
    const { data, error, isLoading } = useSWR(`/api/user/dashboard/todays-followups`, fetcher);
    ...
    const columns: Column<DataItem>[] = [
        {
            ...
            accessor: (item) => (
                <div className="flex items-center gap-2 group">
                    <Link href={`/user/todays-followups/${item.inquiry.id}`}><span className="truncate">{item.inquiry?.name}</span></Link>
        ...
        },
        {
            ...
            accessor: (item) => (
                item.inquiry?.phone ? (<div className="flex items-center gap-2 group">
                    <Link href={`/user/todays-followups/${item.inquiry.id}`}>
        ...
        <section>
            ...
            <div >
                <DataTableComponent
                    ...
                    detail={(item) => `/user/todays-followups/${item.inquiry.id}`}
