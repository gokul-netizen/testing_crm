...
export default function Page() {
    ...
    const { data, error, isLoading } = useSWR(`/api/user/dashboard/upcoming-followups`, fetcher);
    ...
    const columns: Column<DataItem>[] = [
        {
            ...
            accessor: (item) => (
                <div className="flex items-center gap-2 group">
                    <Link href={`/user/upcoming-followups/${item.inquiry.id}`}> <span className="truncate">{item.inquiry?.name}</span></Link>
        ...
        },
        {
            ...
            accessor: (item) => (
                item.inquiry?.phone ? (<div className="flex items-center gap-2 group">
                    <Link href={`/user/upcoming-followups/${item.inquiry.id}`}>
        ...
        },
        {
            ...
            accessor: (item) => (
                <div className="flex items-center gap-2 group relative ">
                    <Link href={`/user/upcoming-followups/${item.inquiry.id}`}>
                        <span className="truncate text-purple-600">{item.followUpStatus === "Assign To" ? `Assign To ${item.assignToName.slice(0, 10)}...` : item.followUpStatus}</span>
                        <div className="flex gap-2">
                            <span className="truncate text-purple-600">{item.date}</span>
                            <span className="truncate text-purple-600">{item.time}</span>
                        </div>
        ...
        <section>
            ...
            <div>
                <DataTableComponent
                    ...
                    detail={(item) => `/user/upcoming-followups/${item.inquiry.id}`}
                    onExcel={() => userExcelData(data, "Up Coming Follow Up")}
