...
export default function Page() {
    ...
    const { data, error, isLoading } = useSWR("/api/admin/dashboard/closed-followup", fetcher);
    ...
    const columns: Column<DataItem>[] = [
        {
            ...
            accessor: (item) => (
                <div className="flex items-center gap-2 group">
                    <Link href={`/admin/dashboard/closed/${item.inquiry.id}`}>
...

        {
            header: "Status", accessor: (item) => (
                <Link href={`/admin/dashboard/closed/${item.inquiry.id}`}>
                    ...
                    <span className="text-red-600">{item.followUpStatus}</span>
                </Link>
            )
        },
        ...
        <section className="p-4 ">
            <div className="flex justify-end">
                <CustomBreadcrumb
                    paths={[
                        ...
                        { label: "Closed", href: `/admin/dashboard/closed` },
            ...
            <div >
                <DataTableComponent
                    ...
                    detail={(item) => `/admin/dashboard/closed/${item.inquiry.id}`}
