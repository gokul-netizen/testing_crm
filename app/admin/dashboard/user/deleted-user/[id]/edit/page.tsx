...
async function fetchAllDomains(): Promise<string[]> {
  try {
    const res = await fetch("/api/domain");
    if (!res.ok) throw new Error("Failed to fetch domains");
    const data = await res.json();
    const alldomains = data.data;
    return alldomains.map((d: { domainName: string }) => d.domainName);
  } catch (err) {
    console.error(err);
    return [];
  }
...
export default function Page() {
    ...
    async function fetchData() {
      ...
      try {
        ...
        setForm({
          ...
          emailTriggerOption : userData.user.emailTriggerOption,
          joining_date: userData.user.joining_date
            ? userData.user.joining_date.split("T")[0]
            : "",
          domains: Array.isArray(userData.user.domains) ? userData.user.domains : [],
        });
        if (userData.user.user_image) {
          setPreview(`/${userData.user.user_image}`);
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      router.push('/admin/dashboard/user/deleted-user')
    }
    else {
    ...
    <div className="w-full bg-white rounded-lg shadow-lg p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-sm text-gray-500">Select Domains</label>
            <MultiSelect
              values={form.domains}
        ...
        <div className="flex justify-start gap-2">
          ...
          <Link
            href="/admin/dashboard/user/deleted-user"
